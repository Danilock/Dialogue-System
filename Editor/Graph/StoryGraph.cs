...
namespace DialogueSystem
{
    public class StoryGraph : EditorWindow
    {
        ...
        private void GenerateToolbar()
        {
            ...
            var containerToLoadField = new ObjectField("Conversation to load:");
            containerToLoadField.objectType = typeof(DialogueConversation);
            containerToLoadField.RegisterValueChangedCallback(
                evt => _conversationToLoad = (DialogueConversation) evt.newValue);
            
            toolbar.Add(containerToLoadField);

            toolbar.Add(new Button(() =>
            {
                RequestDataOperation(false);
                fileNameTextField.SetValueWithoutNotify(_conversationToLoad.name);
                fileNameTextField.MarkDirtyRepaint();
                _fileName = _conversationToLoad.name;
            }) {text = "Load Data"});
        ...
        private void RequestDataOperation(bool save)
        {
            if (!string.IsNullOrEmpty(_fileName))
            {
                ...
                if (save)
                    ...
                    saveUtility.LoadNarrative(_conversationToLoad);
            }
            else
            {
